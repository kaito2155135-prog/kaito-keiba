           ...
           while i < len(lines):
               ...
               if line.isdigit() and 1 <= int(line) <= 18:
                   ...
                   for bl in block_lines:
                       # 馬名の取得強化 (データベースや固有のキーワード、記号を除去しつつ次の行や該当行を綺麗に拾う)
                       if "データベース" in bl:
                           cleaned_name = bl.replace("のデータベース", "").strip().replace("◎", "").replace("〇", "").replace("▲", "").replace("△", "").strip()
                           for prefix in ["牡", "牝", "セ", "セン"]:
                               if cleaned_name.startswith(prefix):
                                   cleaned_name = cleaned_name[len(prefix):].lstrip()
                                   if cleaned_name and cleaned_name[0].isdigit():
                                       cleaned_name = cleaned_name[1:].lstrip()
                           if cleaned_name:
                               h_name = cleaned_name
                       elif any(s in bl for s in ["牡", "牝", "セン", "セ"]) and len(bl) <= 10:
                           # 性別と年齢の正確な抽出 (例: "牡4" や "牝3" など)
                           if "牝" in bl:
                               sex = "牝"
                           elif "セ" in bl or "セン" in bl:
                               sex = "セン"
                           else:
                               sex = "牡"
                           for char in bl:
                   ...
                   # ブロック内から正規の馬名マスターにマッチしやすい候補を探す
                   clean_h_name = h_name.replace("外", "").replace("地", "").replace("◎", "").replace("〇", "").replace("▲", "").replace("△", "").strip()
                   for prefix in ["牡", "牝", "セ", "セン"]:
                       if clean_h_name.startswith(prefix):
                           clean_h_name = clean_h_name[len(prefix):].lstrip()
                           if clean_h_name and clean_h_name[0].isdigit():
                               clean_h_name = clean_h_name[1:].lstrip()

                   for bl in block_lines:
                       candidate = bl.replace("◎", "").replace("〇", "").replace("▲", "").replace("△", "").strip()
                       for prefix in ["牡", "牝", "セ", "セン"]:
                           if candidate.startswith(prefix):
                               candidate = candidate[len(prefix):].lstrip()
                               if candidate and candidate[0].isdigit():
                                   candidate = candidate[1:].lstrip()
                       if "データベース" not in candidate and not any(s in candidate for s in ["牡", "牝", "セ"]) and "人気" not in candidate:
                           if len(candidate) >= 2 and not candidate.replace('.', '', 1).isdigit():
                               clean_h_name = candidate
                               break
