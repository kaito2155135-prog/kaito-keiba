...
st.write("大容量RAWデータ直読み版！過去5年分データ合体＆ブリンカー対応や！🔥")
...
# --- 共通：GitHubのRAWデータから直接・確実に大容量ファイルを読み込む ---
# ※カレントディレクトリにある場合はローカル、なければRAWのURLから読み込む安全設計
jockey_win_rates = {}
df_m_auto = pd.DataFrame()
dfs = []

# ファイルごとのパス（またはRAWのURL）
# まずローカルにあるか確認し、ない場合はGitHubのURLを指定することも可能やけど、
# まずは pandas の読み込みエラー（メモリ不足やエンコーディング）を防ぐための対策を入れる
files = ['keiba_master_data_part1.csv', 'keiba_master_data_part2.csv', 'keiba_master_data.csv']

for f in files:
    if os.path.exists(f):
        try:
            # 試しに最初の数行ではなく、全部をチャンク（または普通に）読み込む
            temp_df = pd.read_csv(f, encoding='utf-8-sig', low_memory=False)
            if len(temp_df) > 10: # 10行より多ければ正常とみなす
                dfs.append(temp_df)
        except Exception as e:
...
with tab1:
    ...
    if len(df_m_auto) > 10:
        ...
        st.warning(f"⚠️ データ数が少なすぎるで（現在 {len(df_m_auto)}行）。ファイルが正しく読めてない可能性があるで！")
        ...
        if st.button("🚀 フルデータでAIを再学習・アップデートする！"):
            try:
                ...
                if len(X) < 5:
                    ...
                    st.balloons()
                    st.success("🎉 20万行フルデータでの最強ガチAIの再学習が完了しました！✨")
