     ...
     if raw_text.strip():
         ...
         while i < len(lines):
             ...
             if line.isdigit() and 1 <= int(line) <= 18:
                 ...
                 matched_hist = {'avg_rank': 5.0, 'best_rank': 5.0, 'avg_time': 0.0, 'avg_last_3f': 35.0, 'avg_corner_4th': 8.0, 'race_count': 0, 'sex': sex}
                 ...
                 input_data_list.append({
                     ...
                     'blinker': 0, 'corner_4th': matched_hist['avg_corner_4th']
                 })
                 i = j - 1
             i += 1
 ...
 if st.button("🚀 ガチ予測を実行する！"):
     ...
     if model is not None:
         try:
             ...
             # 【修正ポイント】後方待機馬（corner_4thが大きい馬）であっても、上がり3Fが速ければ減点されず、逆に末脚がプラス評価されるようにスコア式を改善
             l3f_bonus = (38.0 - df_input['last_3f']).clip(lower=0) * 2.0
             # 後方からの差し・追い込みを評価するため、4角位置のマイナス補正を緩和し、オッズや人気の評価バランスを適正化
     ...
     for idx, row in df_input.iterrows():
         ...
         h_corner = row.get('corner_4th', 8.0)

         st.write(f"**第 {idx+1} 位**: 馬番 {u_num} 🐴 {h_sex}{h_age} **{h_name}** (予測勝率: **{h_prob:.2f}%** / 直近平均着順: {h_avg:.1f}着 / 通過順[1角->4角]: {h_corner:.1f} / 上がり3F: {h_l3f:.1f}秒 / オッズ: {h_odds}倍 / 騎手: {h_jockey})")
