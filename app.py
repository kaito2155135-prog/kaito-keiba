...
st.write("メモリ最適化・超安定学習版！過去5年分データ合体＆ブリンカー対応や！🔥")
    ...
    if model is None:
        ...
        for i in range(num_horses):
            ...
            with st.expander(f"馬番 {auto_umaban} の予測データ", expanded=(i < 3)):
                ...
                with col1:
                    horse_name = st.text_input(f"馬名", "", placeholder="例: アーモンドアイ", key=f"p_name_{i}")
                ...
                with col2:
                    ...
                    r_jockey = st.text_input(f"騎手名", "", placeholder="例: ルメール", key=f"p_jockey_{i}")
