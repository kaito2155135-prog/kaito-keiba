...
def load_and_process_master_data():
       ...
       try:
           ...
           if not df_m.empty:
               ...
               for c in df_m.columns:
                   ...
                   elif clean_c in ['開催', '場所', 'Place', 'place']: col_mapping[c] = 'place'
   ...
   if 'last_3f' in df_m.columns:
       df_m['last_3f'] = pd.to_numeric(df_m['last_3f'], errors='coerce').fillna(35.0)
   else:
       df_m['last_3f'] = 35.0
   ...
   if 'condition' in df_m.columns:
       df_m['condition'] = df_m['condition'].astype(str).str.strip()
   else:
       df_m['condition'] = '良'
   ...
   if 'name' in df_m.columns:
       ...
       if 'condition' in df_m.columns:
           c_grouped = df_m.groupby(['name', 'condition']).agg(
               avg_rank=('rank', 'mean'),
               avg_last_3f=('last_3f', lambda x: x[x > 0].mean() if len(x[x > 0]) > 0 else 35.0),
               race_count=('rank', 'count')
           )
           for (h_name, cond), row in c_grouped.iterrows():
               clean_name = str(h_name).strip()
               if clean_name not in horse_condition_features:
                   horse_condition_features[clean_name] = {}
               horse_condition_features[clean_name][str(cond)] = {
                   'avg_rank': row['avg_rank'] if not np.isnan(row['avg_rank']) else 5.0,
                   'avg_last_3f': row['avg_last_3f'] if not np.isnan(row['avg_last_3f']) else 35.0,
                   'race_count': row['race_count']
               }
       ...
       if 'place' in df_m.columns:
           p_grouped = df_m.groupby(['name', 'place']).agg(
               avg_rank=('rank', 'mean'),
               avg_last_3f=('last_3f', lambda x: x[x > 0].mean() if len(x[x > 0]) > 0 else 35.0),
               race_count=('rank', 'count')
           )
           for (h_name, pl), row in p_grouped.iterrows():
               clean_name = str(h_name).strip()
               if clean_name not in horse_place_features:
                   horse_place_features[clean_name] = {}
               horse_place_features[clean_name][str(pl)] = {
                   'avg_rank': row['avg_rank'] if not np.isnan(row['avg_rank']) else 5.0,
                   'avg_last_3f': row['avg_last_3f'] if not np.isnan(row['avg_last_3f']) else 35.0,
                   'race_count': row['race_count']
               }

   return df_m, jockey_win_rates, horse_history_features, horse_distance_features, horse_condition_features, horse_place_features

df_m_auto, jockey_win_rates, horse_history_features, horse_distance_features, horse_condition_features, horse_place_features = load_and_process_master_data()
       ...
       if raw_text.strip():
           ...
           while i < len(lines):
               ...
               if line.isdigit() and 1 <= int(line) <= 18:
                   ...
                   for bl in block_lines:
                       candidate = bl.replace("◎", "").replace("〇", "").replace("▲", "").strip()
                       for m_name in horse_history_features.keys():
                           if candidate == m_name or (len(m_name) >= 2 and m_name in candidate):
                               clean_h_name = m_name
                               break
                       if clean_h_name != h_name.replace("外", "").replace("地", "").strip():
                           break
                   ...
                   if clean_h_name in horse_distance_features:
                       ...
                       if float(p_distance) in d_dict:
                           ...
                           if abs(closest_dist - float(p_distance)) <= 400:
                               matched_hist['avg_rank'] = (matched_hist['avg_rank'] + d_dict[closest_dist]['avg_rank']) / 2
                               matched_hist['avg_last_3f'] = (matched_hist['avg_last_3f'] + d_dict[closest_dist]['avg_last_3f']) / 2

                   # --- 馬場状態（良・稍重・重・不良）別の適性補正 ---
                   if clean_h_name in horse_condition_features and p_condition in horse_condition_features[clean_h_name]:
                       cond_data = horse_condition_features[clean_h_name][p_condition]
                       matched_hist['avg_rank'] = (matched_hist['avg_rank'] * 0.6) + (cond_data['avg_rank'] * 0.4)
                       matched_hist['avg_last_3f'] = (matched_hist['avg_last_3f'] * 0.6) + (cond_data['avg_last_3f'] * 0.4)

                   # --- 開催場所別の適性補正 ---
                   if clean_h_name in horse_place_features and p_place in horse_place_features[clean_h_name]:
                       place_data = horse_place_features[clean_h_name][p_place]
                       matched_hist['avg_rank'] = (matched_hist['avg_rank'] * 0.7) + (place_data['avg_rank'] * 0.3)
                       matched_hist['avg_last_3f'] = (matched_hist['avg_last_3f'] * 0.7) + (place_data['avg_last_3f'] * 0.3)
   ...
   if not df_m_auto.empty:
       ...
       if st.button("🚀 AIを再学習・アップデートする！"):
           try:
               ...
               if 'place' not in df_train.columns:
                   df_train['place'] = '東京'
