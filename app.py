...
st.write("クラウド自動保存対応！スマホだけでデータ蓄積から予測まで完結するで！✨")
...
with tab2:
    ...
    st.write("結果を入力してボタンを押すだけで、クラウド上に直接データが蓄積されるで！✨")
    ...
    if st.button("🚀 クラウドのマスターCSVにこの結果を追加する！"):
        ...
        st.info("💡 ページを再読み込み（リロード）すると、予測タブや再学習タブにこの最新データが即座に反映されるで！")

with tab3:
    st.subheader("🧠 クラウド上のマスターデータでAIを再学習させる")
    ...
    if os.path.exists(MASTER_FILENAME):
        ...
        if st.button("🚀 このクラウドデータでAIを今すぐ再学習・更新する！"):
            try:
                ...
                if len(X) < 5:
                    ...
                    st.balloons()
                    st.success("🎉 AIの再学習が完了しました！新しいAIモデルがクラウド上で更新されました！✨")
                    st.info("💡 AIモデルも自動保存されたので、予測モデルがパワーアップしたで！")

            except Exception as e:
                st.error("⚠️ 学習中にエラーが発生しました:")
                st.write(e)
    else:
        st.warning("⚠️ クラウド上にマスターデータ（keiba_master_data.csv）がまだありません。「レース結果をマスターに追加」から最初のデータを保存してね！")
