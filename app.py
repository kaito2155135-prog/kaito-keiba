...
st.write("スマホ1台で「フルスペック予測 ➔ マスターCSVに追記 ➔ AIの再学習」を完結させよう！")
    ...
    if model is None:
        ...
        for i in range(num_horses):
            with st.expander(f"馬番 {i+1} の予測用データ", expanded=(i < 3)):
                ...
                with col1:
                    r_waku = st.number_input(f"枠番", min_value=1, max_value=8, value=1, key=f"p_waku_{i}")
                    r_umaban = st.number_input(f"馬番", min_value=1, max_value=18, value=i+1, key=f"p_umaban_{i}")
                    horse_name = st.text_input(f"馬名", f"馬名{i+1}", key=f"p_name_{i}")
                ...
                with col2:
                    ...
                    r_trainer = st.text_input(f"調教師名", "調教師名", key=f"p_trainer_{i}")
        ...
        if st.button("🚀 このメンバーで勝率を予測する！"):
            ...
            # AIが学習時に使う数値特徴量（必要に応じてモデルに合わせて選別されます）
            # ここではオッズ、人気、斤量、年齢、枠番、馬番をAIに渡します
            X_pred = df_input[['odds', 'popularity', 'weight', 'age', 'waku', 'umaban']].fillna(0)
            ...
            try:
                ...
                if hasattr(model, "predict_proba"):
                    ...
                    for idx, row in df_input.iterrows():
                        st.write(f"**第 {idx+1} 位**: 🐴 **{row['name']}** (予測勝率: **{row['win_prob']:.2f}%** / オッズ: {row['odds']}倍 / 騎手: {row['jockey']})")
                else:
...
with tab2:
    ...
    with col_r1:
        ...
        race_number = st.number_input("レース番号 (R)", min_value=1, max_value=12, value=11)
    with col_r2:
    ...
    if train_file is not None:
        ...
        if st.button("🚀 このデータでAIを再学習・更新する！"):
            try:
                ...
                features = ['odds', 'popularity', 'weight', 'age', 'waku', 'umaban']
