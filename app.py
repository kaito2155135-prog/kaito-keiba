...
st.write("ベースは直近5走平均＆競馬場トラック一致！さらに『昇級初戦・初ダート・初芝の過剰人気馬が凡走する人間の感覚』＆『ご提示の6つの穴馬条件』をAIのスコアに自動反映する進化版！🔥")
...
with tab1:
            ...
            while i < len(lines):
                ...
                if line.isdigit() and 1 <= int(line) <= 18:
                    ...
                    matched_hist = {
                        ...
                        'prev_rank': 1.0, 'prev_track': '芝', 'prev_place': '東京',
                        'is_diff_course_or_track': False, 'is_prev_bad_rank': False,
                        'prev_race_high_level_score': 0, 'bloodline_score': 0.0
                    }
                    ...
                    if not sub_df_m.empty:
                        ...
                        if not h_group.empty:
                            ...
                            prev_place = str(last_row.get('place', '東京'))

                            matched_hist['prev_rank'] = prev_rank
                    ...
                    input_data_list.append({
                        ...
                        'is_prev_bad_rank': matched_hist['is_prev_bad_rank'],
                        'is_diff_course_or_track': matched_hist['is_diff_course_or_track'],
                        'prev_race_high_level_score': matched_hist['prev_race_high_level_score'],
                        'bloodline_score': matched_hist['bloodline_score']
                    })
                    i = j - 1
                i += 1
            ...
            for idx, l in enumerate(lines[:18]):
                ...
                if len(clean_l) >= 2 and not clean_l.isdigit():
                    input_data_list.append({
                        ...
                        'is_first_dirt': False, 'is_first_turf': False,
                        'is_first_blinker': False, 'heavy_track_wins_count': 0,
                        'is_prev_bad_rank': False, 'is_diff_course_or_track': False,
                        'prev_race_high_level_score': 0, 'bloodline_score': 0.0
                    })
    ...
    if st.button("🚀 ガチ予測を実行する！"):
        ...
        is_diff_course = df_input['is_diff_course_or_track'].fillna(False)
        diff_course_bonus = np.where(is_diff_course, 3.0, 0.0)

        odds_vals = df_input['odds'].fillna(10.0)
        longshot_bonus = np.where(odds_vals >= 20.0, 6.0, np.where(odds_vals >= 10.0, 3.0, 0.0))

        high_level_bonus = df_input['prev_race_high_level_score'].fillna(0) * 1.5
        blood_bonus = df_input.get('bloodline_score', 0.0) * 2.0
        ...
        if model is not None:
            try:
                ...
                score = rank_score + l3f_bonus + reverse_bonus + promo_penalty + dirt_penalty + turf_penalty + heavy_track_bonus + prev_bad_bonus + diff_course_bonus + longshot_bonus + high_level_bonus + blood_bonus + df_input['jockey_win_rate'] * 2.0 + (1.0 / np.log1p(df_input['odds'])) * 1.5
        else:
            rank_score = (15.0 - df_input['past_avg_rank']).clip(lower=-10.0) * 4.0
            l3f_bonus = (38.0 - df_input['last_3f']).clip(lower=0) * 2.0
        ...
        st.balloons()
        st.subheader("🎯 ガチAI予測結果ランキング（穴馬条件・自動補正版）")
