      ...
      if raw_text.strip():
          ...
          while i < len(lines):
              ...
              if line.isdigit() and 1 <= int(line) <= 18:
                  ...
                  # ブロック全体からマスターデータの馬名が含まれているか直接スキャンして抽出
                  found_master_name = None
                  for bl in block_lines:
                      clean_bl = bl.replace("--", "").strip()
                      for m_name in horse_history_features.keys():
                          if m_name in clean_bl or clean_bl == m_name or clean_bl.replace(" ", "") == m_name.replace(" ", ""):
                              found_master_name = m_name
                              break
                      if found_master_name:
                          break

                  if found_master_name:
                      h_name = found_master_name
                  else:
                  ...
                  for idx_b, bl in enumerate(block_lines):
                      if "データベース" in bl:
                          ...
                          if len(parts) > 0:
                              ...
                              if left_part and not found_master_name:
                                  h_name = left_part
