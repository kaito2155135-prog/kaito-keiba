...
st.write("スマホ1台で「予想 ➔ マスターCSVに追記 ➔ AIの再学習」を完結させよう！")
    ...
    if train_file is not None:
        ...
        if st.button("🚀 このデータでAIを再学習・更新する！"):
            try:
                ...
                # --- AI学習の簡易セットアップ ---
                # 例として 'rank'（着順）が 1（勝ち）かそれ以外（0）かを予測するタスクにする場合
                df_train['target'] = (df_train['rank'] == 1).astype(int)
               
                # 特徴量として使う数値カラム
                features = ['odds', 'popularity', 'weight', 'distance', 'age', 'waku', 'umaban']
