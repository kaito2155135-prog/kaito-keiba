...
st.write("騎手の勝率自動算出＆フルスペック予測で、自分だけの最強AIを回そう！")
...
with tab1:
    ...
    st.markdown("---")
    st.write("📂 騎手の過去勝率を自動計算するために、お持ちの『マスターCSV』をここにアップロードしてください（任意ですが推奨）")
    pred_master_file = st.file_uploader("📂 マスターCSVをアップロード（騎手勝率の算出用）", type=["csv"], key="pred_master_up")

    # 騎手勝率の辞書を作る関数
    jockey_win_rates = {}
    if pred_master_file is not None:
        try:
            df_m_temp = pd.read_csv(pred_master_file, encoding='utf-8-sig')
            if 'jockey' in df_m_temp.columns and 'rank' in df_m_temp.columns:
                # 騎手ごとの総騎乗数と1着回数を計算
                jockey_stats = df_m_temp.groupby('jockey').agg(
                    total=('rank', 'count'),
                    wins=('rank', lambda x: (x == 1).sum())
                )
                # 勝率を計算（最低限のデータがある前提、ない場合はデフォルト0.08）
                for jock, row in jockey_stats.iterrows():
                    if row['total'] > 0:
                        jockey_win_rates[jock] = row['wins'] / row['total']
        except Exception as e:
            st.warning("⚠️ マスターファイルの読み込みに失敗しましたが、予測は続行できます。")
    ...
    if model is None:
        ...
        for i in range(num_horses):
            ...
            with st.expander(f"馬番 {auto_umaban} の予測データ", expanded=(i < 3)):
                ...
                # 騎手勝率の自動紐付け（マスターにデータがあればその勝率、なければ平均的な0.08をセット）
        ...
        if st.button("🚀 この条件とメンバーで勝率を予測する！"):
            ...
            try:
                ...
                st.warning("⚠️ エラーが発生しました（AIモデルの期待する列名と違う可能性があります。「AIの再学習」タブで最新データで再学習を行ってください）:")
                st.write(e)
    ...
    if train_file is not None:
        ...
        if st.button("🚀 このデータでAIを再学習・更新する！"):
            try:
                ...
                # 再学習のときにも、マスターデータから各行の騎手の勝率を自動算出するロジックを組み込む
                ...
                if len(X) < 5:
                    ...
                    st.info("💡 ダウンロードした『keiba_ai_model.pkl』をGitHubにアップロードし直せば、AIがさらに賢くなったものにアップデートされるで！")
