...
st.write("ファイル名指定・確実読み込み版！過去5年分データ合体＆ブリンカー対応や！🔥")
...
with tab1:
    ...
    if len(df_m_auto) > 0:
        ...
        st.warning("⚠️ マスターデータ（part1.csv等）が見つかりません。GitHubのファイル名を確認してね！")
        ...
        if st.button("🚀 フルデータでAIを再学習・アップデートする！"):
            try:
                ...
                if len(X) < 5:
                    ...
                    st.balloons()
                    st.success("🎉 最強ガチAIの再学習が完了しました！これで予測もバッチリ動くで！✨")

            except Exception as e:
                st.error("⚠️ 学習中にエラーが発生しました:")
                st.write(e)
    else:
        st.warning("⚠️ データが読み込めません。「keiba_master_data_part1.csv」と「part2.csv」がGitHubにちゃんと言い値で入っているか確認してね！")
