...
st.write("過去5年分の分割データ合体 ＆ ブリンカー（B）対応の完全版や！🔥")
...
# --- 共通：分割されたマスターデータの自動読み込み＆合体 ＆ 騎手勝率の計算 ---
PART1_FILENAME = 'keiba_master_data_part1.csv'
PART2_FILENAME = 'keiba_master_data_part2.csv'
MASTER_FILENAME = 'keiba_master_data.csv' # 追加・手動入力用

jockey_win_rates = {}
df_m_auto = pd.DataFrame()

dfs = []
if os.path.exists(PART1_FILENAME):
    try:
        dfs.append(pd.read_csv(PART1_FILENAME, encoding='utf-8-sig'))
    except Exception:
        pass
if os.path.exists(PART2_FILENAME):
    try:
        dfs.append(pd.read_csv(PART2_FILENAME, encoding='utf-8-sig'))
    except Exception:
        pass
if os.path.exists(MASTER_FILENAME):
    try:
        dfs.append(pd.read_csv(MASTER_FILENAME, encoding='utf-8-sig'))
    except Exception:
        pass
...
with tab1:
    ...
    if len(df_m_auto) > 0:
        st.success(f"📂 5年分データ合体完了（総データ数: {len(df_m_auto)}行 / 騎手データ反映済み✨）")
    else:
        st.info("💡 ヒント: パートファイルが見当たりません。「keiba_master_data_part1.csv」と「part2」がGitHubにあるか確認してね！")
...
with tab3:
    st.subheader("🧠 5年分データ ＋ 追加データでガチAIを再学習させる")
   
    if len(df_m_auto) > 0:
        st.success(f"📂 合算データ読み込み成功！学習データ総数: **{len(df_m_auto)} 行**")

        if st.button("🚀 5年分フルデータでAIを再学習・アップデートする！"):
            try:
                ...
                if len(X) < 5:
                    ...
                    st.balloons()
                    st.success("🎉 過去5年分＋ブリンカー考慮の最強ガチAIの再学習が完了しました！✨")

            except Exception as e:
                st.error("⚠️ 学習中にエラーが発生しました:")
                st.write(e)
    else:
        st.warning("⚠️ パートファイルが読み込めません。")